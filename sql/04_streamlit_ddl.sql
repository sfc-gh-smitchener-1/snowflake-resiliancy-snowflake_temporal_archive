...
-- Semantic views - show a placeholder if SEMANTIC schema doesn't exist yet
CREATE OR REPLACE VIEW TEMPORAL_ARCHIVE.STREAMLIT.VW_SEMANTIC_VIEWS AS
SELECT 
    'ACCOUNT_USAGE' AS SOURCE_SCHEMA,
    'QUERY_HISTORY_SEMANTIC' AS VIEW_NAME,
    'SEMANTIC' AS VIEW_TYPE,
    'Query history analysis view' AS DESCRIPTION,
    TRUE AS IS_ACTIVE,
    CURRENT_TIMESTAMP() AS CREATED_AT
WHERE EXISTS (
    SELECT 1 FROM TEMPORAL_ARCHIVE.INFORMATION_SCHEMA.SCHEMATA 
    WHERE SCHEMA_NAME = 'SEMANTIC'
)
UNION ALL
SELECT 
    'ACCOUNT_USAGE' AS SOURCE_SCHEMA,
    'WAREHOUSE_USAGE_SEMANTIC' AS VIEW_NAME,
    'SEMANTIC' AS VIEW_TYPE,
    'Warehouse credit usage analysis' AS DESCRIPTION,
    TRUE AS IS_ACTIVE,
    CURRENT_TIMESTAMP() AS CREATED_AT
WHERE EXISTS (
    SELECT 1 FROM TEMPORAL_ARCHIVE.INFORMATION_SCHEMA.SCHEMATA 
...
SELECT 
    'ACCOUNT_USAGE' AS SOURCE_SCHEMA,
    'LOGIN_HISTORY_SEMANTIC' AS VIEW_NAME,
    'SEMANTIC' AS VIEW_TYPE,
    'Login and security analysis' AS DESCRIPTION,
    TRUE AS IS_ACTIVE,
    CURRENT_TIMESTAMP() AS CREATED_AT
WHERE EXISTS (
    SELECT 1 FROM TEMPORAL_ARCHIVE.INFORMATION_SCHEMA.SCHEMATA 
