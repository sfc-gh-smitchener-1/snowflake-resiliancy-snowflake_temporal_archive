/*
================================================================================
SNOWFLAKE TEMPORAL ARCHIVE - SCD TYPE 2 LOAD (PARAMETERIZED TEMPLATE)
================================================================================
...
Run this script after 01_initial_setup.sql.
...
ALTER TABLE {{DATABASE_NAME}}.ARCHIVE.LOAD_LOG ADD COLUMN IF NOT EXISTS RUN_ID NUMBER;
ALTER TABLE {{DATABASE_NAME}}.ARCHIVE.LOAD_LOG ADD COLUMN IF NOT EXISTS LOAD_STRATEGY VARCHAR(30);
...
CREATE TABLE IF NOT EXISTS {{DATABASE_NAME}}.ARCHIVE.WATERMARK_STATE (
...
CREATE OR REPLACE TABLE {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (
...
TRUNCATE TABLE {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY;

-- =============================================================================
-- ACCOUNT_USAGE: APPEND_ONLY views (immutable event/history data)
-- =============================================================================
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
...
-- =============================================================================
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY) VALUES
...
-- EXTERNAL_ACCESS_HISTORY has no timestamp column (only QUERY_ID, HOSTNAMES, STATUS,
-- IP, SOURCE/TARGET_CLOUD/REGION, SENT/RECEIVED_BYTES) so it cannot use APPEND_ONLY watermarking
('ACCOUNT_USAGE', 'EXTERNAL_ACCESS_HISTORY', 'FULL_COMPARE');

-- DEACTIVATED: These views do not exist, are secure objects not accessible under
-- {{ADMIN_ROLE}} role, or have been renamed/removed by Snowflake.
-- Re-activate if Snowflake re-introduces them: UPDATE VIEW_REGISTRY SET IS_ACTIVE = TRUE WHERE ...
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, IS_ACTIVE, LOAD_STRATEGY) VALUES
...
-- 5 views are ACTIVE by default because SEMANTIC.ORGANIZATION_ANALYTICS (see
-- 03_semantic_layer.sql) is built directly on their archive tables. They only
-- return rows on an ORGADMIN-enabled account; on a standalone/member account
-- they will safely return 0 rows (not an error) and add a few minutes per run.
-- If you do not need org-wide analytics, set IS_ACTIVE = FALSE for these 5 to
-- speed up RUN_SCD_LOAD(), and instead drop SEMANTIC.ORGANIZATION_ANALYTICS.
-- All other cross-org views remain DEACTIVATED (0 rows, 3-8 min each to query).
-- To re-activate any of them: UPDATE VIEW_REGISTRY SET IS_ACTIVE = TRUE WHERE SOURCE_SCHEMA = 'ORGANIZATION_USAGE' AND SOURCE_VIEW = '...';
-- =============================================================================
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, IS_ACTIVE, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
('ORGANIZATION_USAGE', 'WAREHOUSE_METERING_HISTORY', TRUE, 'APPEND_ONLY', 'START_TIME', NULL),
...
('ORGANIZATION_USAGE', 'METERING_DAILY_HISTORY', TRUE, 'APPEND_ONLY', 'USAGE_DATE', NULL),
('ORGANIZATION_USAGE', 'RATE_SHEET_DAILY', FALSE, 'APPEND_ONLY', 'DATE', NULL),
('ORGANIZATION_USAGE', 'REMAINING_BALANCE_DAILY', TRUE, 'APPEND_ONLY', 'DATE', NULL),
('ORGANIZATION_USAGE', 'USAGE_IN_CURRENCY_DAILY', TRUE, 'APPEND_ONLY', 'USAGE_DATE', NULL),
('ORGANIZATION_USAGE', 'CONTRACT_ITEMS', FALSE, 'APPEND_ONLY', 'CONTRACT_MODIFIED_DATE', NULL),
('ORGANIZATION_USAGE', 'ACCOUNTS', TRUE, 'SOFT_DELETE_MUTABLE', 'ALTERED_ON', 'ACCOUNT_NAME'),
...
-- =============================================================================
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, IS_ACTIVE, LOAD_STRATEGY, WATERMARK_COLUMN) VALUES
...
-- DEACTIVATED: CALLER_GRANTS_TO_ROLES is not a real SNOWFLAKE.ACCOUNT_USAGE view
-- (404s in current Snowflake docs) - kept here disabled in case it is a typo for a
-- future view name.
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, IS_ACTIVE, LOAD_STRATEGY) VALUES
('ACCOUNT_USAGE', 'CALLER_GRANTS_TO_ROLES', FALSE, 'FULL_COMPARE');

-- =============================================================================
-- ACCOUNT_USAGE: Views added by Snowflake since this registry was first built.
-- Snowflake continuously adds new ACCOUNT_USAGE views; this block brings the
-- registry up to date with the views documented at
-- https://docs.snowflake.com/en/sql-reference/account-usage as of this update.
-- NOT included (intentionally skipped as deprecated/superseded):
--   SNAPSHOTS, SNAPSHOT_SETS, SNAPSHOT_STORAGE_USAGE -> use BACKUPS, BACKUP_SETS,
--     BACKUP_STORAGE_USAGE instead (Dec 2025 WORM backups GA renamed these).
--   CORTEX_FUNCTIONS_QUERY_USAGE_HISTORY -> deprecated, no timestamp column,
--     superseded by CORTEX_AI_FUNCTIONS_USAGE_HISTORY (registered below).
-- =============================================================================

-- Cortex / AI usage
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
('ACCOUNT_USAGE', 'ANOMALIES_DAILY', 'APPEND_ONLY', 'DATE', 'ANOMALY_ID'),
('ACCOUNT_USAGE', 'CORTEX_AGENT_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'CORTEX_AISQL_USAGE_HISTORY', 'APPEND_ONLY', 'USAGE_TIME', NULL),
('ACCOUNT_USAGE', 'CORTEX_AI_FUNCTIONS_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'CORTEX_AI_GUARDRAILS_USAGE_HISTORY', 'APPEND_ONLY', 'USAGE_TIME', NULL),
('ACCOUNT_USAGE', 'CORTEX_CODE_CLI_USAGE_HISTORY', 'APPEND_ONLY', 'USAGE_TIME', NULL),
('ACCOUNT_USAGE', 'CORTEX_CODE_SNOWSIGHT_USAGE_HISTORY', 'APPEND_ONLY', 'USAGE_TIME', NULL),
('ACCOUNT_USAGE', 'CORTEX_PROVISIONED_THROUGHPUT_USAGE_HISTORY', 'APPEND_ONLY', 'INTERVAL_START_TIME', NULL),
('ACCOUNT_USAGE', 'CORTEX_SEARCH_BATCH_QUERY_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'CORTEX_SEARCH_SERVING_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'SNOWFLAKE_COCO_USAGE_HISTORY', 'APPEND_ONLY', 'USAGE_TIME', NULL),
('ACCOUNT_USAGE', 'SNOWFLAKE_COWORK_USAGE_HISTORY', 'APPEND_ONLY', 'USAGE_TIME', NULL);

INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY) VALUES
-- No documented timestamp column - current-state / no reliable watermark
('ACCOUNT_USAGE', 'AI_GATEWAY_USAGE_HISTORY', 'FULL_COMPARE'),
('ACCOUNT_USAGE', 'CORTEX_REST_API_RATE_LIMIT_POLICIES', 'FULL_COMPARE');

-- Native App / DBT project usage
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
('ACCOUNT_USAGE', 'APPLICATION_CALLBACK_HISTORY', 'APPEND_ONLY', 'STARTED_ON', NULL),
('ACCOUNT_USAGE', 'APPLICATION_CONFIGURATIONS', 'SOFT_DELETE_MUTABLE', 'MODIFIED_ON', 'ID'),
('ACCOUNT_USAGE', 'APPLICATION_CONFIGURATION_VALUE_HISTORY', 'SOFT_DELETE_MUTABLE', 'MODIFIED_ON', 'ID'),
('ACCOUNT_USAGE', 'APPLICATION_REMOTE_OPERATION_HISTORY', 'APPEND_ONLY', 'COMPLETED_ON', NULL),
('ACCOUNT_USAGE', 'APPLICATION_SPECIFICATIONS', 'SOFT_DELETE_MUTABLE', 'STATUS_UPDATED_ON', 'ID'),
('ACCOUNT_USAGE', 'APPLICATION_SPECIFICATION_STATUS_HISTORY', 'APPEND_ONLY', 'STATUS_UPDATED_ON', NULL),
('ACCOUNT_USAGE', 'DBT_PROJECT_EXECUTION_HISTORY', 'APPEND_ONLY', 'QUERY_START_TIME', 'QUERY_ID');

-- Storage / backup usage
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
('ACCOUNT_USAGE', 'ARCHIVE_STORAGE_DATA_RETRIEVAL_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'BLOCK_STORAGE_HISTORY', 'APPEND_ONLY', 'USAGE_DATE', NULL),
('ACCOUNT_USAGE', 'BLOCK_STORAGE_SNAPSHOTS', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED_ON', 'SNAPSHOT_ID'),
('ACCOUNT_USAGE', 'STORAGE_LIFECYCLE_POLICIES', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED_ON', 'ID'),
('ACCOUNT_USAGE', 'STORAGE_LIFECYCLE_POLICY_HISTORY', 'APPEND_ONLY', 'END_TIME', NULL),
('ACCOUNT_USAGE', 'STORAGE_REQUEST_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL);

INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY) VALUES
-- Current-state byte-count snapshot, no date/time column
('ACCOUNT_USAGE', 'BACKUP_STORAGE_USAGE', 'FULL_COMPARE');

-- Data governance / data quality / data movement
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
('ACCOUNT_USAGE', 'COLUMN_QUERY_PRUNING_HISTORY', 'APPEND_ONLY', 'INTERVAL_START_TIME', NULL),
('ACCOUNT_USAGE', 'DATA_CLASSIFICATION_HISTORY', 'APPEND_ONLY', 'CLASSIFIED_ON', NULL),
('ACCOUNT_USAGE', 'DATA_MOVEMENT_POLICIES', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'ID'),
('ACCOUNT_USAGE', 'DATA_MOVEMENT_POLICY_RULES', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'ID'),
('ACCOUNT_USAGE', 'DATA_MOVEMENT_VIOLATIONS', 'APPEND_ONLY', 'TIMESTAMP', 'QUERY_ID'),
('ACCOUNT_USAGE', 'DATA_QUALITY_MONITORING_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'JOIN_POLICIES', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'POLICY_ID'),
('ACCOUNT_USAGE', 'OBJECT_ACCESS_REQUEST_HISTORY', 'APPEND_ONLY', 'TIMESTAMP', NULL);

INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY) VALUES
-- Reference/mapping views and current-state snapshots with no reliable watermark
('ACCOUNT_USAGE', 'CONTACT_REFERENCES', 'FULL_COMPARE'),
('ACCOUNT_USAGE', 'DATA_METRIC_FUNCTION_EXPECTATIONS', 'FULL_COMPARE'),
('ACCOUNT_USAGE', 'DATA_MOVEMENT_RULE_REFERENCES', 'FULL_COMPARE'),
('ACCOUNT_USAGE', 'NETWORK_RULE_REFERENCES', 'FULL_COMPARE'),
('ACCOUNT_USAGE', 'PRIVACY_BUDGETS', 'FULL_COMPARE'),
('ACCOUNT_USAGE', 'TRUST_CENTER_FINDINGS', 'FULL_COMPARE');

-- Semantic Views metadata (catalog objects for CREATE SEMANTIC VIEW definitions)
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
('ACCOUNT_USAGE', 'SEMANTIC_DIMENSIONS', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'SEMANTIC_DIMENSION_ID'),
('ACCOUNT_USAGE', 'SEMANTIC_FACTS', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'SEMANTIC_FACT_ID'),
('ACCOUNT_USAGE', 'SEMANTIC_METRICS', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'SEMANTIC_METRIC_ID'),
('ACCOUNT_USAGE', 'SEMANTIC_RELATIONSHIPS', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'SEMANTIC_RELATIONSHIP_ID'),
('ACCOUNT_USAGE', 'SEMANTIC_TABLES', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'SEMANTIC_TABLE_ID'),
('ACCOUNT_USAGE', 'SEMANTIC_VIEWS', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'SEMANTIC_VIEW_ID');

-- Catalog objects (mutable)
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
('ACCOUNT_USAGE', 'EVENT_ROUTING_TABLES', 'SOFT_DELETE_MUTABLE', 'MODIFIED_ON', 'ID'),
('ACCOUNT_USAGE', 'HYBRID_TABLES', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'ID'),
('ACCOUNT_USAGE', 'INDEXES', 'SOFT_DELETE_MUTABLE', 'DELETED', 'ID'),
('ACCOUNT_USAGE', 'INDEX_COLUMNS', 'SOFT_DELETE_MUTABLE', 'DELETED', 'ID'),
('ACCOUNT_USAGE', 'SHARES', 'SOFT_DELETE_MUTABLE', 'MODIFIED_ON', 'NAME'),
('ACCOUNT_USAGE', 'TYPES', 'SOFT_DELETE_MUTABLE', 'LAST_ALTERED', 'TYPE_ID');

INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY) VALUES
-- Structured-type metadata with no timestamp column
('ACCOUNT_USAGE', 'ELEMENT_TYPES', 'FULL_COMPARE'),
('ACCOUNT_USAGE', 'FIELDS', 'FULL_COMPARE'),
('ACCOUNT_USAGE', 'OUTBOUND_PRIVATELINK_ENDPOINTS', 'FULL_COMPARE');

-- Compute / service usage history
INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY, WATERMARK_COLUMN, UNIQUE_KEY_COLUMN) VALUES
('ACCOUNT_USAGE', 'DOCUMENT_AI_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'ICEBERG_STORAGE_OPTIMIZATION_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'INTERNAL_DATA_TRANSFER_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'INTERNAL_STAGE_NETWORK_ACCESS_HISTORY', 'APPEND_ONLY', 'EVENT_TIMESTAMP', 'EVENT_ID'),
('ACCOUNT_USAGE', 'MODEL_SERVING_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'ONLINE_FEATURE_TABLE_REFRESH_HISTORY', 'APPEND_ONLY', 'REFRESH_START_TIME', NULL),
('ACCOUNT_USAGE', 'OPENFLOW_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'POSTGRES_COMPUTE_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'POSTGRES_STORAGE_USAGE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'QUERY_ACCELERATION_ELIGIBLE', 'APPEND_ONLY', 'START_TIME', 'QUERY_ID'),
('ACCOUNT_USAGE', 'SNOWFLAKE_APP_RUNTIME_COMPUTE_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL),
('ACCOUNT_USAGE', 'SNOWPIPE_STREAMING_FILE_MIGRATION_HISTORY', 'APPEND_ONLY', 'START_TIME', NULL);

INSERT INTO {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW, LOAD_STRATEGY) VALUES
-- QUERY_METERING_HISTORY rows are updated in place until the query completes
-- (Adaptive Warehouse per-query metering), so the SOFT_DELETE_MUTABLE hash-based
-- SCD2 compare handles the in-flight-to-final transition correctly.
('ACCOUNT_USAGE', 'QUERY_METERING_HISTORY', 'SOFT_DELETE_MUTABLE'),
-- No reliable single-column watermark documented
('ACCOUNT_USAGE', 'SEARCH_OPTIMIZATION_BENEFITS', 'FULL_COMPARE');
...
CREATE OR REPLACE PROCEDURE {{DATABASE_NAME}}.ARCHIVE.LOAD_VIEW_ARCHIVE(
...
BEGIN
    ...
    v_target_fqn := '{{DATABASE_NAME}}.' || v_target_schema || '.' || v_target_table;
    v_temp_table := '{{DATABASE_NAME}}.' || v_target_schema || '.' || v_target_table || '_STG';
    
    -- Get load strategy from registry
    SELECT LOAD_STRATEGY, WATERMARK_COLUMN 
    INTO :v_load_strategy, :v_watermark_column
    FROM {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY
        ...
        IF (v_watermark_column IS NOT NULL AND v_rows_inserted > 0) THEN
            BEGIN
                ...
                IF (v_new_watermark IS NOT NULL) THEN
                    INSERT INTO {{DATABASE_NAME}}.ARCHIVE.WATERMARK_STATE 
        ...
        LET evolve_cur CURSOR FOR 
            ...
            WHERE c.TABLE_SCHEMA = :v_schema AND c.TABLE_NAME = :v_view
              AND c.COLUMN_NAME NOT IN (
                  ...
                  FROM {{DATABASE_NAME}}.INFORMATION_SCHEMA.COLUMNS ac
    ...
    IF (v_watermark_column IS NOT NULL) THEN
        BEGIN
            ...
            FROM {{DATABASE_NAME}}.ARCHIVE.WATERMARK_STATE
    ...
    IF (v_load_strategy = 'APPEND_ONLY' AND v_last_watermark IS NOT NULL 
        ...
        BEGIN
            ...
            CLOSE wm_c2;
            IF (v_new_watermark IS NOT NULL) THEN
                MERGE INTO {{DATABASE_NAME}}.ARCHIVE.WATERMARK_STATE ws
...
CREATE OR REPLACE PROCEDURE {{DATABASE_NAME}}.ARCHIVE.LOAD_VIEW_ARCHIVE_WITH_RETRY(
    p_source_schema VARCHAR,
    p_source_view VARCHAR,
...
BEGIN
    ...
    WHILE (v_retry_count < p_max_retries AND NOT v_success) DO
        BEGIN
            -- Attempt the load
            CALL {{DATABASE_NAME}}.ARCHIVE.LOAD_VIEW_ARCHIVE(
                :p_source_schema,
                :p_source_view
            ) INTO v_result;
    ...
    ELSE
        RETURN OBJECT_CONSTRUCT(
            ...
            'target', '{{DATABASE_NAME}}.' || p_source_schema || '.' || p_source_view || '_ARCHIVE',
            'status', 'error',
            'error', v_last_error,
...
CREATE OR REPLACE PROCEDURE {{DATABASE_NAME}}.ARCHIVE.RUN_SCD_LOAD()
RETURNS VARIANT
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
BEGIN
    ...
    SELECT COALESCE(MAX(RUN_ID), 0) + 1 INTO :v_run_id FROM {{DATABASE_NAME}}.ARCHIVE.LOAD_LOG;
    ...
    WHILE (v_source_schema IS NOT NULL) DO
        ...
        INSERT INTO {{DATABASE_NAME}}.ARCHIVE.LOAD_LOG (
        ...
        VALUES (
            ...
            '{{DATABASE_NAME}}.' || :v_source_schema || '.' || :v_source_view || '_ARCHIVE',
            :v_log_rows_upd,
            :v_log_rows_ins,
            :v_log_status,
            :v_log_error,
...
CREATE OR REPLACE TASK {{DATABASE_NAME}}.ARCHIVE.TASK_SCD_LOAD_MORNING
    WAREHOUSE = {{WAREHOUSE_NAME}}
    SCHEDULE = 'USING CRON 0 {{MORNING_LOAD_HOUR}} * * * {{TIMEZONE}}'
    COMMENT = 'Morning SCD load. Processes all views from VIEW_REGISTRY.'
AS
    CALL {{DATABASE_NAME}}.ARCHIVE.RUN_SCD_LOAD();

CREATE OR REPLACE TASK {{DATABASE_NAME}}.ARCHIVE.TASK_SCD_LOAD_EVENING
    WAREHOUSE = {{WAREHOUSE_NAME}}
    SCHEDULE = 'USING CRON 0 {{EVENING_LOAD_HOUR}} * * * {{TIMEZONE}}'
    COMMENT = 'Evening SCD load. Processes all views from VIEW_REGISTRY.'
AS
    CALL {{DATABASE_NAME}}.ARCHIVE.RUN_SCD_LOAD();

ALTER TASK {{DATABASE_NAME}}.ARCHIVE.TASK_SCD_LOAD_MORNING RESUME;

ALTER TASK {{DATABASE_NAME}}.ARCHIVE.TASK_SCD_LOAD_EVENING RESUME;
...
SELECT 'SCD load setup complete. ' || 
    (SELECT COUNT(*) FROM {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY) || ' total views (' ||
    (SELECT COUNT(*) FROM {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY WHERE IS_ACTIVE = TRUE) || ' active, ' ||
    (SELECT COUNT(*) FROM {{DATABASE_NAME}}.ARCHIVE.VIEW_REGISTRY WHERE IS_ACTIVE = FALSE) || ' deactivated).' AS STATUS;
...
ALTER TABLE IF EXISTS {{DATABASE_NAME}}.ACCOUNT_USAGE.COLUMNS_ARCHIVE 
    CLUSTER BY ("_IS_CURRENT", "_LOADED_AT");
ALTER TABLE IF EXISTS {{DATABASE_NAME}}.ACCOUNT_USAGE.QUERY_HISTORY_ARCHIVE 
    CLUSTER BY ("_IS_CURRENT", "_LOADED_AT");
ALTER TABLE IF EXISTS {{DATABASE_NAME}}.ACCOUNT_USAGE.AGGREGATE_QUERY_HISTORY_ARCHIVE 
    CLUSTER BY ("_IS_CURRENT", "_LOADED_AT");
ALTER TABLE IF EXISTS {{DATABASE_NAME}}.ACCOUNT_USAGE.ACCESS_HISTORY_ARCHIVE 
    CLUSTER BY ("_IS_CURRENT", "_LOADED_AT");
ALTER TABLE IF EXISTS {{DATABASE_NAME}}.ACCOUNT_USAGE.AGGREGATE_ACCESS_HISTORY_ARCHIVE 
    CLUSTER BY ("_IS_CURRENT", "_LOADED_AT");
ALTER TABLE IF EXISTS {{DATABASE_NAME}}.ACCOUNT_USAGE.TABLES_ARCHIVE 
    CLUSTER BY ("_IS_CURRENT", "_LOADED_AT");
ALTER TABLE IF EXISTS {{DATABASE_NAME}}.ACCOUNT_USAGE.VIEWS_ARCHIVE 
    CLUSTER BY ("_IS_CURRENT", "_LOADED_AT");
