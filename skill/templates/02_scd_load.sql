/*
================================================================================
SNOWFLAKE TEMPORAL ARCHIVE - SCD TYPE 2 LOAD (PARAMETERIZED)
================================================================================
...
CREATE OR REPLACE TABLE {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.VIEW_REGISTRY (
...
TRUNCATE TABLE {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.VIEW_REGISTRY;

-- Insert ALL known ACCOUNT_USAGE views (as of 2024)
INSERT INTO {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.VIEW_REGISTRY (SOURCE_SCHEMA, SOURCE_VIEW) VALUES
-- Core metadata views
...
CREATE OR REPLACE PROCEDURE {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.LOAD_VIEW_ARCHIVE(
...
CREATE OR REPLACE PROCEDURE {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.LOAD_VIEW_ARCHIVE_WITH_RETRY(
    p_source_schema VARCHAR,
    p_source_view VARCHAR,
...
BEGIN
    ...
    WHILE (v_retry_count < p_max_retries AND NOT v_success) DO
        BEGIN
            -- Attempt the load
            CALL {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.LOAD_VIEW_ARCHIVE(
                :p_source_schema,
                :p_source_view
            ) INTO v_result;
...
CREATE OR REPLACE PROCEDURE {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.RUN_SCD_LOAD()
RETURNS VARIANT
LANGUAGE SQL
EXECUTE AS CALLER
AS
...
DECLARE
    ...
    cur CURSOR FOR 
        SELECT 
            SOURCE_SCHEMA,
            SOURCE_VIEW
        FROM {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.VIEW_REGISTRY
...
BEGIN
    ...
    INSERT INTO {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.LOAD_LOG (
...
CREATE OR REPLACE TASK {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.TASK_SCD_LOAD_MORNING
    WAREHOUSE = {{WAREHOUSE_NAME}}
    SCHEDULE = 'USING CRON 0 {{MORNING_LOAD_HOUR}} * * * {{TIMEZONE}}'
    COMMENT = 'Morning SCD load at {{MORNING_LOAD_HOUR}}:00 {{TIMEZONE}}. Processes all views from VIEW_REGISTRY.'
AS
    CALL {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.RUN_SCD_LOAD();

CREATE OR REPLACE TASK {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.TASK_SCD_LOAD_EVENING
    WAREHOUSE = {{WAREHOUSE_NAME}}
    SCHEDULE = 'USING CRON 0 {{EVENING_LOAD_HOUR}} * * * {{TIMEZONE}}'
    COMMENT = 'Evening SCD load at {{EVENING_LOAD_HOUR}}:00 {{TIMEZONE}}. Processes all views from VIEW_REGISTRY.'
AS
    CALL {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.RUN_SCD_LOAD();

ALTER TASK {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.TASK_SCD_LOAD_MORNING RESUME;

ALTER TASK {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.TASK_SCD_LOAD_EVENING RESUME;

-- =============================================================================
-- VERIFICATION
-- =============================================================================

SHOW TASKS IN SCHEMA {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}};
...
FROM {{DATABASE_NAME}}.{{ARCHIVE_SCHEMA}}.VIEW_REGISTRY WHERE IS_ACTIVE = TRUE;
